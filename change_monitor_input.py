...
def set_input_source(input:str):
    feature_value = 0
    input = input.lower()
    if input == 'hdmi':
        feature_value = 0x11
    ...
    for handle in _iter_physical_monitors():
        set_vcp_feature(handle, 0x60, feature_value) #Change input to HDMI

if __name__ == '__main__':
    # get the input source as a command line argument
    import sys
    input_source = sys.argv[1]

    if input_source == 'hdmi' or input_source == 'dp':
